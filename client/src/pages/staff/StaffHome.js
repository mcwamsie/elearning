...
const StaffHome = ({profile}) => {
    ...
    useEffect(() => {
        async function fetchData() {
            setLoading(true)
            try {
                let res = await AxiosInstance.get('/staff/lectures')
    ...
    return (
        <>
            {loading && <Loading/>}
            <MainBreadcrumb text={"Staff Home"} icon={"home"}/>
                    ...
                    <div className="col-12 mb-4">
                        <div className="card h-100">
                            <div
                                ...
                                <Link to={"/staff/personal/lectures/new"} className="btn btn-light">
                                    <i className="fas fa-plus-circle"/>
                                    <span className="ms-2">New Lecture</span>
                                </Link>
                            </div>
                                            ...
                                            lectures.map(({topic, moduleCode, moduleName, startDate, lectureCode}) => (
                                                <tr key={lectureCode}>
                                                    ...
                                                    <td className="p-1">
                                                        ...
                                                        <hr/>
                                                        <a href={`${HOST_URL}/room/staff-link?roomId=${lectureCode}&staffId=${profile.id}`} style={{width: 120}}
...
export default connect(map_state_to_props, {})(StaffHome)