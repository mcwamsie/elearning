...
const StudentSubmittedQuiz = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            try {
                setLoading(true)
                setError(null)
                let res = await AxiosInstance.get(`/student/quiz/submit/${params.id}`)
                setQuiz(res.data)
    ...
    return (
        <>
            {loading && <Loading/>}
            <MainBreadcrumb text={`Student Submitted Quiz`} icon={"puzzle-piece"}/>
...
export default StudentSubmittedQuiz